...
<img src="https://github.com/jphacks/D_2109/wiki/images/trim-pdf-all.png" />
...
.pyファイルのそれぞれにおいて、どういった修正がどの程度入ったかが分かります。
...
<img src="https://github.com/jphacks/D_2109/wiki/images/trim-pdf-reslut.png" />
...
### 基盤の工夫
- マネージド型のAWSリソースのみを利用しているため、可用性・スケーラビリティに優れる
- サーバレスで実現しているため、WebサーバやAPサーバのMiddleWareの準備が不要
- CloudFrontを前段に置くことで、htmlなどの静的コンテンツの取得(from S3)とAPIの取得(from API Gatway)を分離、ネットワークの利用効率が向上
  今回はVueを用いており、クライアントサイドなレンダリングであるため、この構成と相性が良い
- デプロイ時にはVueでビルドしたファイルをS3に転送するだけでよく、手軽
