...
def scan_indent_config(lst, op_indent):
    ...
    for row_no, line in enumerate(lst_cp, 1):
        ...
        if str.endswith(':'):
            ...
            #[trim] Warning: 変数Exception:: 大文字を含められません.
            except Exception:
                pass
    ...
    def check_lst(self, lst):
        ...
        for line in lst:
            ...
            for word in words_lst:
                if word == '':
                    pass
                #[trim] Warning: 変数RESERVED_WORDS:: 大文字を含められません.
                elif word in RESERVED_WORDS:
                    pass
                #[trim] Warning: 変数OTHER_WORDS:: 大文字を含められません.
...
def scan_style_count_word(lst, op_count_word):
    ...
    for line in lst:
        ...
        if len(line)>= op_count_word["length"] + 1 and (not match_flag):
            ...
            # [trim] Warning: 1行あたりの行数は最大120文字です.適切な位置で折り返してください.
...
def lambda_handler(event, context):
    ...
    INFO_MES_LIST_CP = []
    i = 0
    #[trim] Warning: 変数True:: 大文字を含められません.
    while True:
        #[trim] Warning: 変数INFO_MES_LIST[i]: 大文字を含められません.
