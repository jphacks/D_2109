    ...
    def check_lst(self, lst):
        ...
        for line in lst: 
            ...
            if sub_paterns: 
                ...
                if self.get_capwords_flag()andself.get_snake_flag(): 
                    # '_'と大文字が両方入っていたらおかしい
            ...
            for word in words_lst: 
                ...
                # 命名規則のチェック
                    ...
                    if re.search('^[A-Z_]+$', word): 
                        print("定数") 
                        pass 
                    elif self.get_capwords_flag()andself.get_snake_flag(): 
                        # '_'と大文字が両方入っていたらおかしい
...
def scan_style_count_word(lst, op_count_word):
    ...
    for line in lst: 
        ...
        if len(line)>=lengthand(notmatch_flag): 
...
def check_operators_space(line:str, method_naming, class_naming):
    ...
    or re.findall(REJEX_CLASS_NAME, line)): 
        ...
        if(not re.findall('\[.*:.*\]', line)): 
            # [trim] Warning: 1行あたりの行数は最大90文字です.適切な位置で折り返してください.
            if(not re.findall('([a-zA-Z0-9]*)(<>|<=|>=|isnot|notin|-=|==|\+=|!=|=|\+|-|\*|/|%|<|>|and|or|not|in|is)([a-zA-Z0-9]*)', line)): 
                line = re.sub( 
                # [trim] Warning: 1行あたりの行数は最大90文字です.適切な位置で折り返してください.
...
def blank_lines(lst, opt):
    ...
    while True: 
        if i + 1 == len(del_lines)andj+1==len(add_lines): 
            print("complete!") 
            break 
