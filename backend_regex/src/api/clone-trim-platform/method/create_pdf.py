...
def create_pdf(rule: dict):
    snake_path = "img/snake_case.png"
    capword_path = "img/CapWords.png"
    tab_path = "img/tab_indent.png"
    rule_array =[["予約語や変数間の空白","img/brank_check.png","classや関数、演算子前後を空白で整形する機能です。"],
    ["インデント設定", "img/indent.png", "インデントに用いる文字と文字数の設定を行います。"],
    ["一行あたりの文字数","img/line_count.png"," 一行あたりの文字数を決めます。"],
    ["クラス・グローバル関数間の間隔","img/global.png","クラス・グローバル関数ブロックの上下を2行空けるかを決めます。"],
    ["クラス内のメソッドブロック間の間隔","img/method.png", "クラス内のメソッドブロックの上下を1行空けるかを決めます。"],
    ["クラスの命名規則","img/CapWords.png","クラスの命名規則を決めます。チェック：CapWords, 未チェック：Snake"],
    ["関数の命名規則","img/CapWords.png","関数の命名規則を決めます。チェック：CapWords, 未チェック：Snake"],
    ["変数の命名規則", "img/snake_case.png","変数の命名規則を決めます。チェック：CapWords, 未チェック：Snake"],
    ["グルーピング", "img/group.png","インポートを種類によって、振り分けを行う機能です。"],
    ["アルファベット順並び替え", "img/alpa.png", "アルファベット順に並び替える機能です。"],
    ]
    ...
    if rule["style_check"]["indent"]["type"] == " ":
        rule_array[1].append(f"インデント: 半角スペース<{rule['style_check']['indent']['num']}文字>")
    elif rule["style_check"]["indent"]["type"] == "\t":
        rule_array[1][1] = tab_path
        rule_array[1].append(f"インデント: タブ文字<\\t>")
